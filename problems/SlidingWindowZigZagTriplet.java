...
public class SlidingWindowZigZagTriplet {
    ...
    public int[] solution(int[] numbers) {
        int slide = 3; // Trabalhando com triplas
        if (numbers.length < slide) return new int[0]; // Retorna vazio se não houver espaço para uma tripla
        ...
        for (int i = 0; i < result.length; i++) { // Ajustando limite correto
            ...
            for (int j = 1; j < slide; j++) { // Processamos a tripla
